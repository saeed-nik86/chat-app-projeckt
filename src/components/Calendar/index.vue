  ...
  <div id="persian-calendar">
    ...
    <div id="vpc_header" slot="header">
      <div id="vpc_date-control">
        <div class="vpc_control-btn" :disabled="isBeforeMin()" @click="subtractPeriod">
          <svg class="d-block" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
        </div>
        ...
        <div class="vpc_control-btn" :disabled="isAfterMax()" @click="addPeriod">
          <svg class="d-block" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
        </div>
        <div v-if="!disableToday" class="vpc_today-btn" :disabled="todayBtnDisable" @click="goToday">امروز</div>
      </div>
            ...
            <template v-for="i in getWeekEvents(week[0].startOf('jWeek'))">
              ...
              >
                <div
                  ...
                  @click="addEvent(i)"
                >
                  <div :style="{'background-color':i.color}" class="vpc_event-ball" />
                  <span v-if="!hideEventTimes" class="vpc_event-start-time">{{ i.startDateTime.format('HH:mm').toPersianDigits() }}<span> - {{ i.endDateTime.format('HH:mm').toPersianDigits() }} </span>
                  </span>
                  <span v-if="!hideEventTimes" class="vpc_event-start-date">{{ i.startDateTime.format('jMM/jDD').toPersianDigits() }}</span>
                  <span class="vpc_event-title">{{ i.title }}</span>
                  <span v-if="!hideEventTimes" class="vpc_event-end-time">{{ i.endDateTime.format('HH:mm').toPersianDigits() }}</span>
                  <span v-if="!hideEventTimes" class="vpc_event-end-date">{{ i.endDateTime.format('jMM/jDD').toPersianDigits() }}</span>
                </div>
              </slot>
            </template>
            <!--DAYS-->
...
export default {
  ...
  methods: {
    addEvent(i) {
      // this.addToCallender(i.title,false,i.description,i.startDateTime,i.endDateTime)
      this.$emit('updateEvent', i.id)
    },
    ...
    addPeriod() {
      ...
      this.$emit('on-page-add', this.currentDate.locale('fa').format('jYYYYjMM'))
    },
    subtractPeriod() {
      ...
      this.$emit('on-page-subtract', this.currentDate.locale('fa').format('jYYYYjMM'))
    },
    goToday() {
    ...
    getWeekEvents(weekStart) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (ep.endDateTime.isBetween(this.$moment(ep.startDateTime), this.$moment(ep.startDateTime).endOf('day'), undefined, '[]')) {
          for (let d = 0; d < 7; d++) {
            if (d === startOffset) {
              let s = 0
    ...
    emitDay(day, $event) {
      this.$emit('updateShiftComp', day.format('jYYYYjMMjDD'))
